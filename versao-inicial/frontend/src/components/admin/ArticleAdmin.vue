...
export default {
    ...
    data: function() {
        return {
            ...
            users: [],
            page: 1, 
            limit: 0, // qts elementos em cada pagina
            count: 0, // quantas paginas será necessario
            fields: [
    ...
    methods: {
        loadArticles() { // metodo pra carregar os artigos
        ...
        loadCategories() { // metodo para carregar as categorias
            const url = `${baseApiUrl}/categories`
            axios.get(url).then(res => {
                this.categories = res.data.map(category => {
                    return { value: category.id, text: category.path } // apenas os dois atributos que é esperado
                })
            })
        },
        loadUsers() { // para disposição os autores dos artigos para seleção do usuario
